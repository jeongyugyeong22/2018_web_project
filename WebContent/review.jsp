...
<%
	...
	String my_item = request.getParameter("item_name");
	...
	String sql = "select * from review order by review_num DESC";
	ResultSet rs = null;
    int count = 0;
    int number = 0;
%>
...
<body>
	...
	<div style="padding:0px 0px 300px;">							<!-- 스크롤 -->
		...
		<div class="content">							<!-- 내용부분 -->
           ...
           <%
			try{
		 	...
		 	%>
            	<tr>
                	<td><%=review_num %></td>
                	<td width="80px"><a href="reviewcontent.jsp?order_num=<%=order_num%>" style="text-decoration:none;"><%=item_name %></a></td>
                	<td><%= mem_name%></td>
                	<td><%=date%></td>
			<%}
			rs.close();
  		...
  		<br><input type="button" class="writebtn" value="글쓰기" onClick="location.href='reviewwrite.jsp?order_num=<%=order_num %>&item_name=<%=my_item%>'">
		</div>
	</div>
	</div>
</body>
