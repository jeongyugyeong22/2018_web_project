...
function pw_find(){
	...
	else if((find.id.value < "0"||find.id.value > "9")&&(find.id.value < "A"|| find.id.value > "Z")&&(find.id.value < "a"||find.id.value > "z")){
		alert("핸드폰 번호에 한글 및 특수문자는 사용하실 수 없습니다.");
	}
	else if(find.mem_name.value==""){
		alert("이름을 입력하세요.");
		find.mem_name.focus();
	}
	else{
			find.action="./pwfindcheck.jsp";
			find.submit();
		}
	}
...
<body>
	...
	<div id="main">												<!-- 메인 내용 -->
		...
		<br>
		<input type="button" class="btn" value="찾기" onClick="javascript:pw_find();"> <!-- 찾기 버튼 -->
